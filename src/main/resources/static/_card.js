...
class Controller {
    constructor() {
        document.body.addEventListener("mousemove", this.mouseMoveCallback.bind(this));
        document.body.addEventListener("mousedown", this.mousePressCallback.bind(this));
        document.body.addEventListener("mouseup", this.mouseReleaseCallback.bind(this));
        this.mouse_x = 0;
        this.mouse_y = 0;
    ...
    mouseMoveCallback(event) {
        ...
        getState().state.checkState("move");
    }
...
class State {
    ...
    isState(state_id) {
        return state_graph[state_id].state.id = this.id;
    }

    checkState(event) {
        if (event === this.change_event) {
            if (state === "flipping" && this.isState("flipping")) {
                let mouse_pos = controller.getMousePosition();
                if (!page.stack.containsCoordinate(mouse_pos)) {
                    advanceStateTo("un-flipping");
                } else {
                    advanceState();
                }
            } else if (state === "dragging" && this.isState("dragging")) {
                let mouse_pos = controller.getMousePosition();
                if (!page.stack.containsCoordinate(mouse_pos)) {
                    return;
                }

                if (zones.stack.containsCoordinate(mouse_pos)) {
                    advanceStateTo("un-flipping");
                    return;
                }

                let stack_pos = zones.stack.getMidpoint();
                if (mouse_pos.x < stack_pos.x) {
                    advanceStateTo("flipped");
                    return;
                }
        ...
        if (event === "move" && state === "basic-flipping" && this.isState("basic-flipping")) {
            advanceStateTo("flipping");
        }
    }
    ...
    applyState(element, flip_angle=false) {
        ...
        State.applyAngle(element, angle);
...
class ActiveStack {
    ...
    containsCoordinate(coordinate) {
        let rect = this.front.getBoundingClientRect();
        if (coordinate.x > rect.right) {
            return false;
        } else if (coordinate.y > rect.bottom) {
            return false;
...
let state_graph = {
    ...
    "basic-flipping": {
        state: new State(0.5, 0.0, "release"),
        next: "flipped",
        prev: "not-flipped"
    },
    "flipping": {
    ...
    "un-flipping": {
        state: new State(0.5, 0.0, "time-elapsed"),
        next: "not-flipped",
        prev: "flipped"
    }
}
...
function advanceStateTo(state_id) {
    state = state_id;
